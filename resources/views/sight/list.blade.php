...
<div class="container-fluid p-5" style="width: 85%; position: relative;">
    ...
    <div class="button-container pb-5">
        <a href="{{ route('sightexecom.create') }}" class="btn btn-primary">Add New Execom Member</a>
    </div>
        ...
        <tbody>
            @foreach ($datas as $data)
                <tr>
                    <td>{{ $data->name }}</td>
                    <td>{{ $data->title }}</td>
                    <td><img src="{{ asset('uploads/images/sightexecoms/' . $data->image) }}" style="height: 100px" alt=""></td>
                    <td>
                        {{ $data->github }}<br>
                        {{ $data->insta }}<br>
                        {{ $data->linkedin }}<br>
                    ...
                    <td>
                        <div class="d-flex">
                            <form method="GET" action="{{ route('sightexecom.edit', ['id' => $data->id]) }}" style="display:inline">
                                @csrf
                            ...
                            <form method="POST" action="{{ route('sightexecom.delete', ['id' => $data->id]) }}" style="display:inline">
                                @csrf
